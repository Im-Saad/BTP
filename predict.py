...
class G4Predictor(nn.Module):
    def __init__(self, seq_length=201, embed_dim=64, num_heads=4, dropout=0.3):
        ...
        encoder_layer = TransformerEncoderLayer(
            d_model=embed_dim, nhead=num_heads, dim_feedforward=256, dropout=dropout, batch_first=True
        )
...
def main(input_csv):
    ...
    out_name = f"predicted_{os.path.basename(input_csv)}"
    df.to_csv(out_name, index=False)

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python predict_g4.py <input_csv>")
    else:
        main(sys.argv[1])
